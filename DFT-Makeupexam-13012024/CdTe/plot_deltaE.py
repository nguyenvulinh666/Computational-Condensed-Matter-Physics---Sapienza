...
if 0 == 1:	
	...
	plt.savefig('./Convergence_test_CdTe/deltaE_mev_Ecut_CdTeNone.png')
	plt.show()

if 1 == 1:	
	# Read the data from energy_ecut.dat
	ecut, etot = [], []
	with open("./Convergence_test_CdTe/energy_summary_convergence_kmesh_CdTe_70.txt", 'r') as file:
	    #next(file)  # skip header
	    for line in file:
	        e, t = line.split()
	        ecut.append(float(e))
	...
	plt.ylim(-5, max(delta_E)+(max(delta_E)-min(delta_E))*0.3)
#	plt.xlim(min(ecut_mid)-10, max(ecut_mid)+10)
	plt.savefig('./Convergence_test_CdTe/deltaE_mev_kmesh_CdTe70.png')
	plt.show()
