...
if 1 == 1:	
	...
	ecut, etot = [], []
	with open("./Results_si.cd/energy_summary_si.cd6.txt", 'r') as file:
	...
	plt.savefig('./Results_si.cd/deltaE_mev_Ecut_si.cd6.png')
	plt.show()

if 1 == 1:	
	# Read the data from energy_ecut.dat
	ecut, etot = [], []
	with open("./Results_si.cd/energy_summary_convergence_kmesh_si.cd_50.txt", 'r') as file:
	    #next(file)  # skip header
	    for line in file:
	        e, t = line.split()
	        ecut.append(float(e))
	...
	plt.ylim(-5, max(delta_E)+(max(delta_E)-min(delta_E))*0.3)
#	plt.xlim(min(ecut_mid)-10, max(ecut_mid)+10)
	plt.savefig('./Results_si.cd/deltaE_mev_kmesh_si.cd50.png')
	plt.show()
