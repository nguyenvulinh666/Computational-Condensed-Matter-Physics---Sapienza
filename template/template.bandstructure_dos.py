...
Ef =  -9.4351 # Remember to update the value!
ne = 8
emin = -24
emax = 20
...
bands = np.genfromtxt( './bands_dos_${prefix}/${prefix}.bands.dat.gnu' )
## Shift all energies by Fermi energy (can be extracted from QE output files)
bands[:,1] = bands[:,1] - Ef
fig = plt.figure(1, [10, 6], dpi=100)

ax1.set_title('${atoms} Band structure', fontsize=fontsize+2)
...
for i in range(num_bands):
    ax1.plot(bands[i*num_pts:i*num_pts+num_pts,0], bands[i*num_pts:i*num_pts+num_pts,1], linestyle = '-',color = 'k', linewidth=2.0)
## Plot Fermi level
...
dos = np.genfromtxt('./bands_dos_${prefix}/${prefix}.dos') 
dos[:,0] = dos[:,0] - Ef

ax2 = plt.subplot(gs[1], sharey=ax1) 

# Plot DOS (Note: x and y are swapped here to plot vertically)
ax2.plot(dos[:,1], dos[:,0], color = 'k', alpha=0.8, linewidth=1, label=r"D($\epsilon$)")
ax2.plot(dos[:,2], dos[:,0], color = 'r', alpha=0.8, linewidth=1, label=r"N($\epsilon$)")
ax2.hlines(0, 0, 20.0, linestyle='--', color='b', label=r"EF") 
...
ax2.legend(loc='upper right', bbox_to_anchor=(0.95, 0.95), borderaxespad=0., ncol=1)
